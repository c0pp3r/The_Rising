...
public:
	...
	int doQueueCommand(CreatureObject* creature, const uint64& target, const UnicodeString& arguments) const {
		...
		if (!player->checkCooldownRecovery("force_lightning_cone2")){
			Time* cdTime = player->getCooldownTime("force_lightning_cone2");
			//Returns -time. Multiply by -1 to return positive
			int timeleft = floor((float)cdTime->miliDifference() /1000) * -1;

			player->sendSystemMessage("Force Lightning Cone 2 to is on Cooldown");
			return GENERALERROR;
		}
		player->addCooldown("force_lightning_cone2", 10 * 1000); //10 second cooldown
